// to compile: clang++-7 -pthread -std=c++17 -o pr5 pr5.cpp
#include <climits>
#include <algorithm>
#include <queue>
#include <sys/time.h>
...
for (int v = 0;v<g.nodes;v++)
{
  ...
  if (dv != INT_MAX)
    for (int i=g.nindex[v];i<g.nindex[v+1];i++)
      {const int n = g.nlist[i];
      const int d = dv+g.eweight[i];
      // check if new lower distance is found
      if (d<dist[n])
      {
        dist[n]=d;
        parent[n]=v;
...
static void MST_prim(const int src, const ECLgraph& g, int* const dist, int* const prev, int* const color)
{
  ...
  while (pq.size() > 0) {
    ...
    for (int i = g.nindex[v]; i < g.nindex[v + 1]; i++) {
      const int n = g.nlist[i];
      const int d = g.eweight[i];
      if (color[n]==0 && dist[n]>d)
      {
        dist[n] = d;
        prev[n]= v;
        pq.push(std::make_pair(dist[n], n));
      }
      // check if color of n is 0 and if dist[n]>weight
      // dist[n]=d
      // update prev
      // push push pq.push(std::make_pair(dist[n], n)); // ^ without negative
    }
  }
}
